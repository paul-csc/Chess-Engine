...
namespace ChessCpp {
constexpr void
Magics::createAllBlockerBitboards(Bitboard movementMask, Bitboard blockers[], int& numPatterns) {
...
constexpr Bitboard Magics::createRookLegalMovesBitboard(Square square, Bitboard blockBitboard) {
    Bitboard moves = 0ULL;
    ...
    s = square + WEST;
    while (s >= 0 && file_of(s) < file_of(square)) {
        moves |= 1ULL << s;
        if (blockBitboard & (1ULL << s))
            break;
        s += WEST;
    ...
    s = square + EAST;
    while (s < SQUARE_NB && file_of(s) > file_of(square)) {
        moves |= 1ULL << s;
        if (blockBitboard & (1ULL << s))
            break;
        s += EAST;
...
constexpr Bitboard Magics::createBishopLegalMovesBitboard(Square square, Bitboard blockBitboard) {
    Bitboard moves = 0ULL;
    int file = file_of(square);
    Square s = square + NORTH_EAST;
    while (s < SQUARE_NB && file_of(s) > file) {
        moves |= 1ULL << s;
        if (blockBitboard & (1ULL << s))
            break;
        s += NORTH_EAST;
...
void Magics::init() {
    ...
    for (Square startSquare = SQ_A1; startSquare < SQUARE_NB; ++startSquare) {
        ...
        for (int i = 0; i < numPatterns; ++i) {
            ...
            RookAttackTable[startSquare][index] =
                createRookLegalMovesBitboard(startSquare, blockerBitboard);
        }
